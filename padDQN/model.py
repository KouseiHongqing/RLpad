...
class TinyNet(nn.Module):
    ...
    def choose_action(self, x,limit):
        x = torch.unsqueeze(torch.FloatTensor(x).to(self.device), 0)
        # 这里只输入一个 sample
...
class DQN(object):
    ...
    def batch_store_transition(self,transition):
        ...
        self.memory = self.memory[-args.memory_capacity:,:]
        self.memory_counter = self.memory.shape[0]

    def learn(self):
        # target net 参数更新
        if self.learn_step_counter % args.target_replace_iter == 0:
        ...
        q_target = b_r + GAMMA * q_next.max(1)[0].view(args.batch_size, 1)    # shape (batch, 1)
...
args={'row_size':3,'col_size':3,'color_size':3}