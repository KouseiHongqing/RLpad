...
LastEditTime: 2021-08-11 17:04:09
'''
import numpy as np
import random
...
class Board():
    ...
    def initBoard(self,Fall=True,Full =False):
        ...
        #随机生成
            self.board = np.random.randint(1,self.colorSize,(self.rowSize,self.colSize))
    ...
    def initBoardnoDup(self,Full = False):
        self.initBoard(Full)
         #重复版面优化
        record = []
        pos= []
        s= self.board.reshape(1,-1)[0]
        for i in s:
            if(not i in record):
                record.append(i)
        ...
        for i in pos:
            ...
            for j in i:
                res[j]=index
        self.board = res.reshape(self.rowSize,self.colSize)
